...
export default function Dashboard() {
    ...
    const dataInsights = useMemo(() => {
        ...
        const bestProduct = Object.entries(prodMap).sort((a, b) => b[1] - a[1])[0] || ['N/A', 0];

        const chanMap = {};
        filteredData.forEach(d => chanMap[d.channel] = (chanMap[d.channel] || 0) + d.revenue);
        const bestChannel = Object.entries(chanMap).sort((a, b) => b[1] - a[1])[0] || ['N/A', 0];

        const dayMap = {};
        filteredData.forEach(d => dayMap[d.date] = (dayMap[d.date] || 0) + d.revenue);
        const bestDay = Object.entries(dayMap).sort((a, b) => b[1] - a[1])[0] || ['N/A', 0];
        ...
        const bestConvChannel = Object.entries(chanConvMap).sort((a, b) => b[1] - a[1])[0] || ['N/A', 0];
    ...
    const generateBusinessInsights = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                alert("Failed to generate AI insights: " + json.error);
            }
        } catch (error) {
            console.error("AI Generation Error:", error);
            alert("Network Error generating AI insights.");
        } finally {
            ...
            <div className="min-h-screen bg-slate-50/50 dark:bg-slate-950/95 transition-colors duration-300">
                <div className="p-4 lg:p-8 flex flex-col gap-6 max-w-[1600px] mx-auto">
                            ...
                            <div className="flex items-center gap-2 w-full md:w-auto">
                                ...
                                >
                                    <optgroup label="2026 Intelligence Fleet" className="bg-slate-900 text-white">
                                        <option value="gemini-2.5-flash">Gemini 2.5 Flash (Core)</option>
                                        <option value="gemini-2.5-pro">Gemini 2.5 Pro (Logic)</option>
                    ...
                    <section className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
                        {[
                            { label: 'Total Gross Revenue', value: formatCurrency(stats.revenue), icon: DollarSign, color: 'text-blue-600', bg: 'bg-blue-50 dark:bg-blue-500/10' },
                            { label: 'Cumulative Orders', value: stats.orders, icon: ShoppingCart, color: 'text-purple-600', bg: 'bg-purple-50 dark:bg-purple-500/10' },
                            { label: 'Net Profit Yield', value: formatCurrency(stats.profit), icon: TrendingUp, color: 'text-emerald-600', bg: 'bg-emerald-50 dark:bg-emerald-500/10' },
                            { label: 'Conversion Rate', value: `${stats.conversion.toFixed(1)}%`, icon: Target, color: 'text-orange-600', bg: 'bg-orange-50 dark:bg-orange-500/10' },
                        ].map((kpi, i) => (
                            <div key={i} className="glass-card group p-5">
                                <div className="flex items-center justify-between">
                                    <span className={`p-2.5 rounded-xl ${kpi.bg} ${kpi.color} transition-transform group-hover:scale-110`}>
                                        <kpi.icon className="w-5 h-5" />
                                    </span>
                                </div>
                                <div className="mt-5">
                                    <p className="uppercase tracking-widest text-[10px] font-black opacity-80 text-slate-500">{kpi.label}</p>
                                    <h3 className="text-2xl lg:text-3xl mt-1 tracking-tight font-bold">{kpi.value}</h3>
                                </div>
                            </div>
                        ))}
                    </section>
                            ...
                            <div className="flex-1 w-full">
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={chartData.trend}>
                                        ...
                                        <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fill: isDarkMode ? '#64748b' : '#94a3b8', fontSize: 10 }} />
                                        <YAxis axisLine={false} tickLine={false} tick={{ fill: isDarkMode ? '#64748b' : '#94a3b8', fontSize: 10 }} tickFormatter={(val) => `$${formatCompact(val)}`} />
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-full">
                                <div className="glass-card p-6 flex flex-col min-h-[300px]">
                                    ...
                                    <div className="flex-1 w-full">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <PieChart>
                                                <Pie data={chartData.byChannel} innerRadius={60} outerRadius={80} paddingAngle={8} dataKey="value">
                                    ...
                                    <div className="flex-1 w-full">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <BarChart data={chartData.byProduct} layout="vertical">
                                                ...
                                                <YAxis dataKey="name" type="category" axisLine={false} tickLine={false} tick={{ fill: isDarkMode ? '#94a3b8' : '#64748b', fontSize: 11, fontWeight: 700 }} width={90} />
                                                <Tooltip formatter={(val) => [formatCurrency(val), 'Revenue']} />
